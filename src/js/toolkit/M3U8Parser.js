...
const matchers = {
  ...
  isComment: (line) =>
    line && line[0] === "#" && !line.startsWith(matchers.tagPrefix),
  isBlank: (line) => line === "",
...
export class M3U8Parser {
  ...
  readSegment(line) {
    ...
    if (parsed === null) {
      console.log("readsegment", "read segment failed", "line:", line);
      return;
    }
    ...
    if (duration < this.tags.TARGETDURATION) {
      console.log(
        "readsegment",
        "Segment duration must be less than the TARGETDURATION",
        "duration:",
        duration
      );
    }
  ...
  readLine(line) {
    ...
    switch (tag.name) {
      ...
      case matchers.allowCache:
        this.tags.ALLOW_CACHE = /\s*(YES|yes)\s*/.test(tag.value)
          ? true
          : false;
        break;
      ...
      case matchers.endList:
        ...
        break;
      default:
        console.log("readLine", "unknow line", "line:", line);
    }
  }
  ...
  addFile(line) {
    if (line[0] === "#") {
      ...
      switch (tag.name) {
        ...
        default:
          console.log("addFile", "Invalid tag.", "tag.name:", tag.name);
      }
      return;
    }
    ...
    if (sd >= 0) {
      ...
      let lastSegement = this.segments[this.segments.length - 1] || {
        sd: 0,
        start: 0,
        end: 0,
      };
    ...
    if (!this._currentSegment.name) {
      this._currentSegment.name = line.substr(
        Math.max(0, line.lastIndexOf("/")),
        line.lastIndexOf(".")
      );
    }
  ...
  parse(source) {
    ...
    if (typeof source !== "string") {
      ...
      console.log("m3u8 file is not text.");
      return;
    }
    this._state = STATE.READING;
    this.discontinuous = false;
    ...
    lines.forEach((line) => {
      ...
      switch (this._state) {
        ...
        case STATE.ADD_FILE:
          ...
          break;
        default:
          console.log("parsing m3u8 get an error, it is unknow state.");
      }
    });
    return this;
  }
